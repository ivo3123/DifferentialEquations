...
$\left.u\right|_{t=0} = \displaystyle\sum_{k=1}^{\infty} C_k \sin(k\pi x) = \phi(x)$
...
$\left.u\right|_{t=0} = \displaystyle\sum_{k=1}^{\infty} C_k \sin(k\pi x) = \sin(3\pi x) - 2\sin(\pi x)$
