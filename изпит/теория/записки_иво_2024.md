...
$$T''(t) + \lambda a^2T(t) = 0 \quad (\#_1)$$

$$X''(x) + \lambda X(x) = 0 \quad (\#_2)$$
...
$\implies X(x)$ и $T(t)$ са решения на $(\#_1)$ и $(\#_2)$ с $\lambda = \dfrac{X''(x_0)}{X(x_0)} = -\dfrac{T''(t_0)}{T(t_0)}$

$\implies (\#_1)$ е в сила за $t \in [0, T_{max}]$.

$\implies (\#_2)$ е в сила за $x \in [0, L]$.
