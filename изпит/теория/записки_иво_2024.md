...
$$
\begin{array}{r l}
I & = \displaystyle\int\limits_{y_1}^{y_2}|f_y'(x, \lambda)|\mathrm{d}\lambda
\newline
& = |\displaystyle\int\limits_{y_1}^{y_2}f_y'(x, \lambda)\mathrm{d}\lambda|
\newline
& = |f(x, y_2) - f(x, y_1)|
\end{array}
$$
...
$\textit{Теорема}$ (Локална теорема за единственост и съществуване): Нека $f(x, y)$ е непрекъсната и липшицова по $y$ в $\Pi := \lbrace(x, y)\in\mathbb{R}:|x - x_0| \le a, |y-y_0| \le b\rbrace$.Тогава съществува единствено решение на задачата на Коши $\begin{cases}y' = f(x, y) \newline y(x_0) = y_0\end{cases}$ в интервала $[x_0-h,x_0+h]$, където $h = \min \left\lbrace a, \dfrac{b}{M} \right\rbrace$ и $M = \displaystyle \max_{\Pi}|f(x, y)|$.
